...
const AmazonUpdateInventory = () => {
    ...
    const [state, setState] = useState(
     ...
     const { merchantskus, merchantskusResult, loaderState } = state
    ...
    const rowSelection = {
        onChange: (selectedRowKeys, selectedRows) => {
            ...
            if (selectedRows.length > 0) {
                selectedRows.map(singleSelectedRow => {
                    let keyArray = Object.keys(singleSelectedRow)
                    let tempObject = {}
                    keyArray.map(sinlgeKey => {
                        if (singleSelectedRow[sinlgeKey].props) {
                            let temp = singleSelectedRow[sinlgeKey].props.children !== undefined ? singleSelectedRow[sinlgeKey].props.children : singleSelectedRow[sinlgeKey].props.value
                            console.log(sinlgeKey, singleSelectedRow[sinlgeKey], temp)
    ...
    const handleChange = (index, key, value, data) => {

        console.log('dataSource', data)
        // let temp  = data[index].key.props.children !== undefined ? data[index].key.props.children : data[index].key.props.value
        //     if(data[index].key[sinlgeKey].props){

        //     let temp  = key[sinlgeKey].props.children !== undefined ? singleSelectedRow[sinlgeKey].props.children : singleSelectedRow[sinlgeKey].props.value
        //     console.log(sinlgeKey, singleSelectedRow[sinlgeKey], temp)

        //   }
        let temp = [...data]
        temp[index][key] = value
        console.log('aa', temp[index][key])

        //   setState({ ...state, dataSource: tempDataSource });
        setState({ ...state, merchantskusResult: temp })
    ...
    const merchantskustable = (merchantskusResult) => {
        ...
        if (merchantskusResult !== '')
            merchantskusResult.map((value, i) => {
                ...
                tempDataSource.push({
                    ...
                    PUPrice: <Input style={{ color: 'black', width: 100 }} type="text" onChange={(PU_price) => { handleChange(i, 'PU_price', PU_price.target.value, merchantskusResult) }} value={PU_price} />,
        ...
        console.log('aaaa', tempDataSource)
                    ...
                    <div>
                        <Cards title="View Merchantsku">
                            ...
                            <div className="table-responsive">
                                <Table pagination={true} dataSource={dataSource} columns={columns} rowClassName={() => 'editable-row'} bordered
