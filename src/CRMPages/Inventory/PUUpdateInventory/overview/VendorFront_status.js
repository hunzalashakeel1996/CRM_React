...
const VendorFront_status = () => {
  ...
  dispatch(getVendorapi()).then(data => {
    ...
    i++;
      ...
      setstate({ plainOptions: plain, loader: false });
  })
  },[plainOptions])
  ...
  const onCheckAllChange = e => {
    setIsVendorCheckedList(e.target.checked ? [...plainOptions] : [])
  ...
  const onListCheckChange = (val, i, isChecked) => {
    ...
    setCheckAll(plainOptions.length===temp.length ? true : false)
...
export default VendorFront_status;