...
const{Regularvendor}=props
  ...
  const onChange = list => {
    setCheckedList(list);
    setIndeterminate(!!list.length && list.length < Ggroupsku.length);
    setCheckAll(list.length === Ggroupsku.length);
  };

  const onCheckAllChange = e => {
    setIsVendorCheckedList(e.target.checked ? [...Ggroupsku] : [])
  ...
  const onListCheckChange = (val, i, isChecked) => {
    ...
    setCheckAll(Ggroupsku.length===temp.length ? true : false)
