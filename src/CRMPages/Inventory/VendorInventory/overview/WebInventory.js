import { Col, Row, Select, Spin, Radio, Checkbox, Divider, List, Popover } from 'antd';
...
const Ggroupsku = ['MERCHANTSKU', 'STYLECODE', 'STYLENAME'];
...
const{webVendor,loader,updateVendor}=props
  ...
  const onCheckAllChange = e => {
    if(e.target.checked){
      updateVendorList = [...webVendor]
    }else{
      updateVendorList = []

    }
    setIsVendorCheckedList(e.target.checked ? [...webVendor] : [])
  ...
  const onListCheckChange = (val, i, isChecked) => {
   username=JSON.parse(localStorage.getItem('user'))
    ...
    setCheckAll(webVendor.length===temp.length ? true : false)
