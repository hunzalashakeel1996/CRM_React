...
import { getRequest_Refund_Tracking_Report } from '../../../redux/apis/DataAction';
...
const RefundReport = (props) => {
  ...
  const getRefundReport = () => {
    ...
    dispatch(getRequest_Refund_Tracking_Report({ orderdatefrom: state.startDate.format('MM/DD/YYYY'), orderdateto: state.endDate.format('MM/DD/YYYY')})).then(data => {
      setstate({ ...state, isLoader: false })
      // console.log('My Data: ', data)
      //downloadFile(data);
      notification.success({
        message: 'Successfull Rendered',
        description: `Successfully Request Refunded Reports From ${state.startDate.format('MM/DD/YYYY')}`,
        onClose: close,
      });
            ...
            <Cards title="Refunded Report">
              ...
              <Row>
                <Col xs={24} style={{ marginBottom: 10 }}>
                  ...
                  <Button size="large" type="primary" onClick={getRefundReport} style={{ marginRight: 10, }} > Search</Button> 
