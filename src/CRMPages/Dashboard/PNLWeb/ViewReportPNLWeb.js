...
import { apiReportOrderWiseWeb, apiReportItemWiseWeb } from '../../../redux/apis/DataAction';
...
const ReportPNLWebView = () => {
  const [form] = Form.useForm();
  const dispatch = useDispatch();
  const userAccess = JSON.parse(localStorage.getItem('userRole'))[0];

  const tabChildBar = JSON.parse(userAccess.top_navigation)['Report PNL Web'];

  const [activeTab, setActiveTab] = useState('OrderPNL');

  let ordertype = ['PU', 'JLC', 'WebALL']
  ...
  const getsummary_report_order_wise = () => {
    ...
    Promise.all([
      ...
      dispatch(apiReportOrderWiseWeb({ dateFormat: dateFormat,orderdatefrom: state.startDate.format('MM/DD/YYYY'), orderdateto: state.endDate.format('MM/DD/YYYY') })),
      dispatch(apiReportItemWiseWeb({dateFormat: dateFormat, orderdatefrom: state.startDate.format('MM/DD/YYYY'), orderdateto: state.endDate.format('MM/DD/YYYY') })),
  ...
  //     // }
  //     //  if ('Web' === e.target.value.toString()) {
              ...
              {dataSourceOrder.length > 0 && <Row style={{ marginTop: 10 }}>
                ...
                <Col span={10} >
                  ...
                  <Row style={{ marginTop: 10 }} >
                    ...
                    <Col span={20}>
                      {/* disabled={isSellerType === 'disabled' ? true : false} */}
...
export default ReportPNLWebView;
